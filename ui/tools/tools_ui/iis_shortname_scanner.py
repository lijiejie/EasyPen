...
class IISShortNameScannerPanel(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(self, parent, -1)
        self.SetBackgroundColour(wx.WHITE)

        lbl_description = wx.StaticText(self, -1, 'IIS short names brute force enumerate exploit')
        ...
        lbl_url = wx.StaticText(self, -1, 'Docs:  https://github.com/lijiejie/IIS_shortname_Scanner')
    ...
    def exploit(self, event):
        ...
        self.result.Clear()
        log_output('IIS Short Name Enumerate exploit: %s' % self.url.GetValue())
        self.process = wx.Process(self)
        self.process.Redirect()
        cmd = sys.executable + " " + os.path.join(conf.root_path, "tools/iis_shortname_scanner/iis_shortname_scan.py ")
        if conf.is_windows_exe:
            cmd = os.path.join(conf.root_path, "tools/iis_shortname_scanner/iis_shortname_scan.exe ")
    ...
    def url_click(self, event):
        webbrowser.open_new_tab('https://github.com/lijiejie/IIS_shortname_Scanner')
        event.Skip()
...
if __name__ == '__main__':
    ...
    panel_git_hack = IISShortNameScannerPanel(panel)
