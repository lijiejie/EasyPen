...
class SwaggerExpPanel(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(self, parent, -1)
        self.SetBackgroundColour(wx.WHITE)

        lbl_description = wx.StaticText(self, -1, 'A Swagger UI API information disclosure exploit')
        ...
        lbl_url = wx.StaticText(self, -1, 'Docs:  https://github.com/lijiejie/swagger-exp')
    ...
    def exploit(self, event):
        ...
        if self.url.GetValue().strip() and \
                not self.url.GetValue().lower().startswith('http') and self.url.GetValue().find('://') < 0:
            self.url.SetValue('http://' + self.url.GetValue())
        self.result.Clear()
        self.url.Enable(False)
        self.btn_exp.Enable(False)
        self.result.Clear()
        log_output('Swagger API exploit: %s' % self.url.GetValue())
        self.process = wx.Process(self)
        self.process.Redirect()
        cmd = sys.executable + " " + os.path.join(conf.root_path, "tools/swagger-exp/swagger-exp.py ")
        if conf.is_windows_exe:
            cmd = os.path.join(conf.root_path, "tools/swagger-exp/swagger-exp.exe ")
    ...
    def url_click(self, event):
        webbrowser.open_new_tab('https://github.com/lijiejie/swagger-exp')
        event.Skip()
...
if __name__ == '__main__':
    ...
    panel_git_hack = SwaggerExpPanel(panel)
