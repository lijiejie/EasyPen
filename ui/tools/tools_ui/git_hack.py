...
class GitHackPanel(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(self, parent, -1)
        self.SetBackgroundColour(wx.WHITE)

        lbl_description = wx.StaticText(self, -1, 'GitHack is a .git folder disclosure exploit.')
        ...
        lbl_url = wx.StaticText(self, -1, 'Docs:  https://github.com/lijiejie/GitHack')
    ...
    def exploit(self, event):
        ...
        self.result.Clear()
        log_output('GitHack exploit: %s' % self.url.GetValue())
        self.process = wx.Process(self)
        self.process.Redirect()
        cmd = sys.executable + " " + os.path.join(conf.root_path, "tools/GitHack/GitHack.py ")
        if conf.is_windows_exe:
            cmd = os.path.join(conf.root_path, "tools/GitHack/GitHack.exe ")
    ...
    def url_click(self, event):
        webbrowser.open_new_tab('https://github.com/lijiejie/GitHack')
        event.Skip()
...
if __name__ == '__main__':
    ...
    panel_git_hack = GitHackPanel(panel)
    sizer = wx.BoxSizer(wx.HORIZONTAL)
