...
from lib.bbscan.lib.common import clear_queue, parse_url, cal_depth, get_domain_sub, is_port_open, scan_given_ports, is_ip_addr
from lib.bbscan.lib.cmdline import parse_args
from lib.bbscan.lib import save_report
from lib.bbscan import lib as conf
...
async def main():
    for input_file in args.input_files:
        if args.host:
            ...
            if args.network == 32 and len(target_list) * 2 < args.left_panel:
                args.left_panel = len(target_list) * 2
        ...
        try:
            ...
            if args.crawler:
                ...
                for _file in input_files:
                    ...
                    conf.tasks_count += 1
                if conf.tasks_count < args.left_panel:
                    args.left_panel = conf.tasks_count
                conf.process_targets_done = True
            else:
                conf.tasks_count = 0
                asyncio.create_task(prepare_targets(target_list))

            all_process = [scan_process() for _ in range(args.left_panel)]
            await q_results.put('%s scan process created' % args.left_panel)
